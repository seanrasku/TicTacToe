...
public class TicTacToe1 {
	...
	*/
	...
	private Map<Position1, Integer> store = new HashMap<>();
	/*
	CONSTANTS:
	*/
	...
	public Best chooseMove(int side, int depth) {
		...
		int bestColumn = -1;
		int value;
		Position1 thisPosition = new Position1(board);
	...
	private class Position1 {
		...
		public boolean equals(Object rhs) {
			...
			if (!(rhs instanceof Position1))
				return false;

			for (int i = 0; i < 3; i++)
				for (int j = 0; j < 3; j++)
					if (board[i][j] != ((Position1) rhs).board[i][j])
						return false;
			return true;
		}
